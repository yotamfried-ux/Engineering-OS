...
| .github/workflows/enforcement-tests.yml | checked | Focused, grouped, per-letter, and C-prefix suite steps surface failures for changed evidence/audit/coverage gates and aggregate groups before the final sweep. |
...
- `validation.actions-checked` — workflow changes were reviewed against the focused/grouped/per-letter/C-prefix enforcement suite steps and aggregate sweep.
...
- connectors: GitHub.
- steps: add checker, tests, PR evidence script call, gap row, audit row, coverage manifest row, focused/grouped/per-letter/C-prefix CI diagnostics, and PR body evidence.
- evidence: target paths, fixtures, PR policy body validation, simulation coverage registration, audit wording validation, focused/grouped/per-letter/C-prefix enforcement suite steps, and CI runs.
- rejected: unrelated target-run evidence, weakening the aggregate enforcement sweep, and unregistered capability ids.
- result: checker path connected to PR policy; usage availability validation now requires yes/no; simulation coverage now requires the operational behavior gate; audit enforced rows avoid deferred wording; changed-gate, grouped, per-letter, and C-prefix suite failures are visible as separate CI steps.
...
- Enforcement-tests keeps the aggregate sweep and adds focused, grouped, per-letter, and C-prefix suite steps before it.
...
- Run grouped, per-letter, and C-prefix enforcement suites.
- Confirm CI.
...
- pre-merge: after commit `735466e`, `enforcement-tests` job logs showed the exact failing assertion was `installed check-pr-review-evidence.sh runs from the target project` inside `test-clean-install-and-usage.sh`; root-caused to the `installed-body.md` fixture missing `## Operational Behavior Evidence`, fixed the fixture, re-ran locally, found a second real bug (the installed checker's `check-operational-behavior-evidence.sh` dependency was never copied into installed targets because `policy-gate-dependencies.tsv` had no row for it), fixed the manifest, trimmed the 9 ad-hoc per-letter/topic enforcement-tests.yml debug steps down to the 4 range groups, and reran the full local `test-*.sh` sweep (79+ suites) clean.
- pre-merge: after commit `735466e`, `workflow-evidence-policy` failed with "pre-merge checkpoint evidence must be introduced or materially updated after the last code/config/test change" because this plan's pre-merge entry was committed in the same commit as the code/test/config fix rather than a strictly later commit; this plan-only follow-up commit (no code/config/test files changed) supplies that required later pre-merge checkpoint.
