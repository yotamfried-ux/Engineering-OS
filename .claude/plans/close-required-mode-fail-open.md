...
- pre-merge: re-verified after the CodeRabbit round. Full enforcement suite 113 suites, 0 failures; CI picks the new suite up automatically because `enforcement-tests.yml:104` globs `scripts/enforcement/tests/test-*.sh`. `test-hook-home-resolution.sh` carries 6 assertions, now asserting the exact documented outcomes rather than loose ones: the bare fixture requires exactly 127 plus its command-not-found diagnostic, and the resolvable fixture runs the unit directly in the same environment and requires the gate to reproduce the documented hook-gate mapping. It was also proven non-vacuous — case 1's assertion returns 127 against the bare form, so a regression to an ungated command fails it, which matters because this change's subject is a test-shaped blind spot. A real `install-policy-gates.sh` run into a fresh git target leaves zero unresolved home forms and a gate-wrapped guard, and installing with the pre-change installer from `main` produced the same 0 portable / 39 absolute counts, so the change is behaviour-preserving for installed targets rather than newly introducing absolute paths. `main` was merged in after PR #270 landed mid-review, so the diff carries only this change's three files. `shellcheck` clean on both changed shell files. All five plan-evidence gates pass.
