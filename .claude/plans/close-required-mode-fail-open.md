...
- pre-merge: full enforcement suite 113 suites, 0 failures — 112 previously plus this PR's new suite, which CI picks up automatically because `enforcement-tests.yml:104` globs `scripts/enforcement/tests/test-*.sh`, so no workflow wiring was needed. `test-hook-home-resolution.sh` carries 6 assertions and was proven non-vacuous rather than assumed to work: case 1's assertion was run against the bare form and returned 127 where it requires 2, so a regression to an ungated command fails it. That check matters here specifically — this PR's subject is a test-shaped blind spot, and a regression that could not fail would have reproduced it. A real `install-policy-gates.sh` run into a fresh git target leaves zero unresolved home forms and a gate-wrapped guard. `shellcheck` is clean on both changed shell files; its one SC2016 finding was on the deliberate literal placeholder and is suppressed with a stated reason rather than silently ignored. All five plan-evidence gates verified green on this exact branch state: `check-workflow-evidence.sh`, `check-connector-evidence.sh`, `check-documentation-asset-evidence.sh`, `enforce-run-trace.sh` and `validate-capability-evidence.sh`.
