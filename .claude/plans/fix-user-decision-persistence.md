...
| Target paths | `CLAUDE.md`; `core/user-decision-policy.md`; `experiments/claude-behavioral-eval/`; `scripts/enforcement/tests/`; `.claude/plans/fix-user-decision-persistence.md` |
...
2. Add a concise mandatory pointer in `CLAUDE.md` and register the new core owner in the navigation and conceptual-ownership tables.
...
5. Add positive and negative fixtures proving one ask passes, repeated asks fail, deferred cross-repo state is accepted, and a materially changed premise may supersede a prior decision.
6. Run the full policy/review loop and merge only on the exact reviewed green head.

## Definition of Done

- [ ] The canonical policy distinguishes unanswered decisions from answered/deferred/blocked decisions.
- [ ] The policy forbids asking the same decision again unless the prior answer is ambiguous, contradictory, or invalidated by a material fact change.
- [ ] Plan Mode/read-only sessions continue in-scope work without requiring a file write solely to remember the answer.
- [ ] Cross-repo actions unavailable in the current workspace become a concrete deferred/blocked handoff, not a repeated prompt.
- [ ] `CLAUDE.md` points to the canonical policy and navigation/orphan ownership remains valid.
- [ ] The behavioral evaluator supports deterministic occurrence limits.
- [ ] A neutral task packet and oracle cover the Project 8-style cross-repo decision loop.
- [ ] Positive and negative regression fixtures pass locally/in CI.
...
- internal: `CLAUDE.md`, `core/workflow.md`, `core/task-router.md`, `core/precedence.md`, PR #247, and the behavioral evaluation harness.
...
- `plan.route-plan-before-write` — this file is the first branch commit, before policy, evaluator, or test changes.
- `source.github-repo-read` — inspected current main and PR #247 through GitHub.
- `validation.policy-change-has-validator` — occurrence-count evaluator coverage and policy fixtures are mandatory in the DoD.
...
- action: inspected main SHA `9c99081cc5476b752837a878c50bfae2e4880b3e`, PR #247, `CLAUDE.md`, `core/workflow.md`, `core/task-router.md`, and behavioral-eval assets.
- result: confirmed that the repeated question is not emitted by a hook; it arises from an unresolved cross-repo checklist item plus a missing answered/deferred decision lifecycle.
...
- connectors: GitHub.
- steps: verify current main and PR #247; trace hook behavior; inspect workflow/task-router and eval harness; create plan-first branch; implement policy and evaluator fixtures; run exact-head review/CI; merge when verified.
- evidence: user screenshot, PR #247, current source, regression fixtures, Actions, and review threads.
...
- start: branch created from exact `main` SHA `9c99081cc5476b752837a878c50bfae2e4880b3e`; this Route Plan is committed before every implementation/test change.
- mid: pending.
