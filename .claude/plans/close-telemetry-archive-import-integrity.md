...
- pre-merge: re-verified after the Codex review round. Full enforcement suite 112 suites, 0 failures; `test-telemetry-archive.sh` carries 26 assertions including 13 negative integrity cases, each asserting a per-file `sha256sum` fingerprint of the whole archive is unchanged after rejection. Each rejection was checked for its actual cause rather than only its exit status: missing boundary reported `handoff boundary position is invalid`; invalid policy reported `invalid telemetry policy identity`; manifest replacement reported `events checksum mismatch`; a symlinked member reported `required file is missing or not regular`; an extra file reported `outside the allowlist`. Validation and archival now act on the same private snapshot, so the bytes validated are the bytes stored. `shellcheck` 0.9.0 reported no findings on the changed shell file. `check-workflow-evidence.sh`, `check-connector-evidence.sh`, `check-documentation-asset-evidence.sh` and `check-capability-staged-changes.sh` all pass. Recorded on PR #268.
