...
| Evidence to check | pre-change, `import-telemetry-run.py` contained no reference to `telemetry_handoff` (it does after this change); `validate_bundle()` already implements checksum, symlink, identity and boundary checks |
...
|---|---|---|
| `scripts/monitoring/import-telemetry-run.py` | read | **Pre-change, on `main` at `ee138b26`:** contained zero references to `telemetry_handoff`. Validation was limited to manifest schema, event schema and banned-field scanning, so checksums and run identity were never compared on the direct import path. In the final tree it imports `telemetry_handoff` and delegates to `validate_bundle()`. |
...
- mid: added `validate_before_mutation()` ahead of every write, wired the four `expected_*` passthroughs, recorded the integrity decision in the archive index, and rebuilt the fixtures as synced bundles via the real `write_handoff_manifest`. Each of the twelve required rejections was then checked for *why* it failed, not just that it failed. That check was initially done by hand and recorded in prose; review showed prose is the wrong place for it, so `reject_unchanged` now takes the expected reason and matches it against the importer's real stderr. An earlier claim here — that `sync-telemetry-run.py` refuses to write handoff metadata for a boundaryless run — was **wrong**: measured, it writes `boundary_position=0` and reseals the checksums, and the validator rejects that value. One layer, not two.
- pre-merge: re-verified after the Codex and CodeRabbit review rounds. Full enforcement suite 112 suites, 0 failures; `test-telemetry-archive.sh` carries 42 assertions, of which 14 are negative integrity cases. Each negative case asserts two things: that a per-file `sha256sum` fingerprint of the whole archive is unchanged after rejection, and **which check fired**, matched against the importer's real stderr. That second assertion replaces a hand-written per-case failure table — a negative test that only asserts "it failed" can pass for the wrong reason, and here it nearly did: `validate_bundle()` compares checksums before boundary position, so the boundaryless fixture had to re-sync unmasked to reach the boundary check at all. Validation and archival act on the same private snapshot, so the bytes validated are the bytes stored. `shellcheck` 0.9.0 reported no findings on the changed shell file. `check-workflow-evidence.sh`, `check-connector-evidence.sh`, `check-documentation-asset-evidence.sh`, `check-capability-staged-changes.sh`, `validate-capability-evidence.sh` and `enforce-run-trace.sh` all pass. The review round's own lesson is recorded at `lessons-learned/bugs/negative-test-passing-for-the-wrong-reason.md` — companion to `unit-level-contract-passing-while-wiring-violates-it.md` from PR #266, one layer further out: that one was an assertion proving the unit but not the wiring, this one an assertion proving a failure but not which check produced it. Recorded on PR #268.
