...
| Evidence to check | telemetry schema, settings hook wiring, target install contract, PR CI logs |
| User decisions required | owner approval before merge |
| Target paths | scripts/monitoring/eos-telemetry-event.sh, scripts/monitoring/eos-telemetry-summary.py, scripts/enforcement/tests/test-eos-telemetry.sh, .claude/settings.json |
...
- decision: Use local trace/span-style metadata records and hook lifecycle wiring.
...
- pre-merge: Final plan evidence recorded after implementation commits.
...
- result: clean branch has final lifecycle evidence.
