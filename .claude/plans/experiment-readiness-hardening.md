...
4. preserve mode semantics: invalid evidence fails in `required` but warns and continues in `best_effort`;
5. copy only validated required files into the selected CI artifact;
6. keep code readiness, first-run evidence, and longitudinal sufficiency as separate claims.
...
| `scripts/monitoring/select-pr-telemetry.py` | validated | Selection rejects symlinked directory boundaries, routes invalid evidence through mode-aware no-candidate handling, and copies only required files. |
...
| `scripts/enforcement/tests/test-telemetry-trust-boundaries.py` | validated | Negative fixtures cover PR policy override, required-file symlinks, `runs/` symlinks, run-directory symlinks, best-effort warning behavior, and extra-file exclusion. |
...
- decision: retain Git handoff plus archive, use an explicit trusted disabled policy for absent-base adoption, preserve `best_effort` warning semantics, and require a real exact-match bundle before experiment success.
...
- `validation.policy-change-has-validator` — focused policy, directory-boundary, mode-semantics, allowlist, and workflow regressions cover the changed controls.
- `validation.actions-checked` — telemetry-handoff-tests run 218 and enforcement-tests run 1316 passed on final code head `d8b56c5e4f444e174c04df0c280e93c6cfcf0422`.
- `validation.coderabbit-policy` — every CodeRabbit/Codex finding was verified and reconciled; merge remains owner-gated.

## Skill Evidence

- `security-review` — applied to trusted policy ownership, environment isolation, symlink traversal, directory containment, mode semantics, metadata-only validation, allowlisted copying, and exact repository/PR identity.
...
- action: compared canonical and target helpers; inspected the rejected full sync; merged #252 after approval; rebuilt #253 on squash commit `5759cac6b0a1602ff225ba789cfaf2536de0a693`; diagnosed CI and review findings; verified final code head `d8b56c5e4f444e174c04df0c280e93c6cfcf0422`.
- result: eleven PR files are present — this plan plus the ten Target paths; telemetry-handoff-tests run 218 and enforcement-tests run 1316 passed, while `scripts/monitoring/select-pr-telemetry.py`, `.github/workflows/pr-policy.yml`, and `scripts/enforcement/tests/test-telemetry-trust-boundaries.py` contain the final security and mode-semantics repairs.
- decision: merging PR #252 enabled the clean rebuild; Project 8 PR #9's rejected full-sync evidence caused selected-file allowlisting; policy and symlink findings caused explicit trusted-default and directory-boundary checks; the best-effort P2 caused mode-aware invalid-evidence handling; exact-head status kept PR #253 blocked until every finding and thread was reconciled.
...
- rating: 5.
- confidence: high.
- outcome: negative fixtures exposed and closed policy-fallback, directory-symlink, and best-effort regression risks before a live experiment.
...
- Invalid directory evidence enters the normal no-candidate path: `required` fails and `best_effort` warns without failing CI.
...
- symlinked `runs/` root in `required` → reject;
- symlinked `runs/` root in `best_effort` → warning and success;
- symlinked run directory → invalid candidate and mode-aware result;
- directory resolving outside the handoff root → invalid candidate and mode-aware result;
...
- Raise every directory-boundary failure before policy handling — rejected because it breaks documented `best_effort` behavior.
...
- hypothesis: combine #252's Git parser, Project 8's reviewed trust controls, an explicit trusted disabled adoption policy, directory-boundary checks, and mode-aware invalid-evidence handling without changing telemetry schema.
- connectors: GitHub for repository, PR, CI, artifacts, and review truth; official vendor documentation read directly.
- steps: audit; compare target/canonical code; create plan-first branch; add hardening and regressions; reconcile audits; merge #252; rebuild #253 on the squash commit; fix test-root and evidence defects; fix trusted-policy fallback and symlinked directory traversal; preserve best-effort semantics; run focused and aggregate suites.
- evidence: PR #253, final code head `d8b56c5e4f444e174c04df0c280e93c6cfcf0422`, telemetry-handoff-tests run 218, enforcement-tests run 1316, CodeRabbit/Codex review threads, Project 8 #7/#9, Engineering OS #252, and official documentation above.
- rejected: blind sync; target-only fork; PR-controlled fallback; symlinked directories/files; whole-directory copy; weakened required mode; over-strict best-effort behavior; duplicate stacked history; OWH-only success.
...
- [x] Required mode fails on invalid directory evidence while best-effort mode warns and succeeds.
- [x] Canonical selector copies only the validated three-file allowlist.
- [x] `scripts/enforcement/tests/test-telemetry-trust-boundaries.py` covers policy fallback, environment override, file symlink, directory symlink, best-effort warning, and allowlist fixtures.
...
- [x] telemetry-handoff-tests run 218 and enforcement-tests run 1316 pass on final code head `d8b56c5e4f444e174c04df0c280e93c6cfcf0422`.
...
- pre-merge: final code commit `d8b56c5e4f444e174c04df0c280e93c6cfcf0422` preserved required-mode blocking and restored best-effort warning semantics; telemetry-handoff-tests run 218 and enforcement-tests run 1316 succeeded, and review thread `PRRT_kwDOS6Ejks6SdnPN` was reconciled and resolved before this final evidence commit.