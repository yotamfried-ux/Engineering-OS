...
plus regressions that execute the malformed shapes, plus the audit entry recording what the
qualification measured about required mode.
...
- pre-merge: extended after a self-review of the downstream diff, because the external reviewer was rate-limited on that head and an unreviewed head is not a reviewed one. Reviewing it myself found two defects in my own work, both fixed in `yotamfried-ux/project-8@3696177`: `hard_gate_problems` let `subprocess.TimeoutExpired` escape while its two sibling functions caught it, so a hook command that hangs crashed the validator instead of failing cleanly — the readable-FIFO shape this PR's lesson explicitly records as unmeasured; and the invocation-binding rule was written out twice, one contract in two places, which is the defect class the validator exists to catch. The audit now records what required mode actually does, including a constraint no gap row covers: a cancelled `SessionEnd` leaves the run un-handed-off and the next session's guard denies every tool call. Measured on Project 8 — 8 local events against a 7-event bundle, then a run with `pre_tool_use` and no `post_tool_use`. No gap status moves here.
- pre-merge: re-verified after the review round, which is the state that differs from the earlier entry. CodeRabbit raised four items on head `0074916`; all four were valid and are addressed. Three were the same defects the local gate reproduction had already found — the DoD named no concrete signal, the mid checkpoint carried no post-work measurement, and the context7 line named no source. The fourth was independent and correct: the declared Target paths omitted `.claude/settings.json` and the lesson file, both of which this plan writes, so `check-plan-scope.sh` had no basis to accept them. A fifth correction came from the same round and is a substantive one — the lesson generalised a single directory measurement into a claim about every readable non-regular node, and called exit 126 a "hard failure" where the measured fact is a non-zero process status rather than a `PreToolUse` denial; both are now limited to what was measured. All three evidence checkers reproduce clean locally against base `4290481`. Full enforcement suite 113 suites, 0 failures; `shellcheck` clean; `test-hook-home-resolution.sh` 8 assertions, the two new ones returning 126 against the pre-change form. Full enforcement suite 113 suites, 0 failures; `shellcheck` clean on both changed shell files. `test-hook-home-resolution.sh` now carries 8 assertions, the two new ones executing the malformed-wrapper shape rather than inspecting rendered text, and both return 126 against the pre-change form, so neither is vacuous. The six focused suites — home resolution, hard-hook fail-closed, hook gate, boundary parity, user-level installer, install-policy-gate coverage — all pass. The boundary-parity suite initially failed because the repository's own checked-in settings still carried the old form; normalising the two bootstrap expressions in place rewrote 28 commands with the hook count unchanged at 38, and the suite passes. The failure class is recorded as a lesson with its regression named, since a guard whose precondition is weaker than the operation it protects is a class rather than a one-off.
