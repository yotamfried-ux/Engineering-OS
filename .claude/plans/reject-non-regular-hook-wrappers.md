...
- decision: tighten all four, add an executing regression for the malformed-wrapper case in both forms, and normalise the two bootstrap expressions in the checked-in `.claude/settings.json` in place so the parity suite's manifest comparison stays consistent without altering the hook set.
