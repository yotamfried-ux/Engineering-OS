...
- decision: chose to reuse the existing checklist and audit findings instead of re-deriving them from scratch; limited this session's real work to the concrete open items that are achievable inside this repo (B1, B4, C1, C2) and updated the checklist's B2 status to reflect that it already shipped on `main`. Kept B3 (the real Project 8 experiment) explicitly gated and unchecked, per the user's instruction not to run another data-collection experiment until every item is resolved with real evidence.
