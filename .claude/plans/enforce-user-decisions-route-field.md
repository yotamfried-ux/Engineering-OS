...
- pre-merge: verified via `pull_request_read`/`get_check_runs` on PR #196 that after the fixture fix (commit 94471bc) `enforcement-tests` (run 28719352855) reports `success`, along with pr-policy, connector-evidence-policy, capability-evidence-policy, plan-policy, documentation-asset-policy, semantic-cleanup-policy, and import-cleanup-policy; this checkpoint update itself is the remaining piece workflow-evidence-policy required before merge is requested from the owner.
