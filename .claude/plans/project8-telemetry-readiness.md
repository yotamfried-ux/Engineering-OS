...
Evidence to check: Project 8 `main`, `.claude/settings.json`, installer output, current-session `events.jsonl`, run-id rotation, exported telemetry bundle, branch-level GitHub Actions history, Operational Work History artifact, and final-head PR reviews/checks
...
10. `docs/operations/project8-telemetry-preflight.md` defines the mandatory fresh-session verification and export sequence for the next experiment.
...
- result: `scripts/install-policy-gates.sh`, `scripts/monitoring/eos-telemetry-session-start.sh`, `scripts/monitoring/require-telemetry-session.sh`, and `scripts/enforcement/tests/test-project8-telemetry-readiness.sh` now contain concrete upstream corrections and regression coverage
- decision: fixed the canonical installer and telemetry lifecycle upstream rather than adding another Project 8-only workaround; Codex P1/P2 findings and the official hook exit-code contract changed the implementation before merge readiness
- target: `.claude/settings.json`, installer, telemetry recorder/session/preflight scripts, `.github/workflows/pr-policy.yml`, and both install/telemetry regression suites
...
- `validation.policy-change-has-validator`: installer/hook/privacy/history changes have executable regression coverage.
...
- mid: corrected direct/custom installation, all-tools lifecycle coverage, session isolation, blocking exit semantics, privacy-safe event metadata, historical CI aggregation, the preflight procedure, and the real Codex findings; final-head validation and final review resolution remain outstanding external gates.
