...
- `plan.route-plan-before-write` — this plan is committed before workflow changes.
- `source.github-repo-read` — GitHub files checked before edits.
- `validation.policy-change-has-validator` — workflow gate will validate policy evidence.
- `validation.coderabbit-policy` — CodeRabbit policy checked.
...
- result: github showed policy evidence was not hard-gated.
- decision: github selected .github/workflows/pr-policy.yml as the enforcement target.
...
- goal: close P2 policy evidence gap.
- hypothesis: structured PR policy evidence blocks vague manual process.
