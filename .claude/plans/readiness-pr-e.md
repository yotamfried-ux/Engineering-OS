...
- pre-merge: after CI caught a real CI-environment-dependent-fixture-premise bug (Experiment 6's PATH guess assumed ruff/pytest absent, false on the actual runner) and Codex flagged Experiment 7 for testing the source hook instead of the installed one, both were fixed (commit ebb6672) and the full suite re-verified green.
- pre-merge: after CodeRabbit's review, the one Major finding (install-policy-gates.sh silently skipped the whole dependency-copy loop when the manifest was missing instead of failing closed, matching the existing hard-fail behavior of the workflow-copy loop above it) and four Nitpick findings (SC2015-style `A && B || C` in test-install-policy-gate-coverage.sh; commit-msg.sh silently no-oping instead of warning when an enforcement script is missing; the EOS_ALLOW_MISSING_TOOLS waiver test not isolating the waiver variable from the CI flag it was paired with; env -i not preserving HOME in Experiment 3's PATH-absence checks, unlike the equivalent Experiment 6 checks) were fixed and the full suite re-verified green.
