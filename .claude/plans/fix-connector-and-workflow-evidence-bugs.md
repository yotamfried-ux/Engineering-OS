...
- pre-merge: Codex's automated PR review found a real gap in the Bug 2 fix (marker exemption filtered `code_idxs` but not `plans` itself, so an inherited branch's own plan file would corrupt `first_plan`); fixed in commit 4773657 with a new regression fixture, verified failing on the pre-fix code and passing after via `git stash`. Full local `enforcement-tests` (83/83) and this PR's own CI `enforcement-tests` check both green on head 4773657; `connector-evidence-policy`, `capability-evidence-policy`, `documentation-asset-policy`, `plan-policy`, `semantic-cleanup-policy`, and `import-cleanup-policy` all green. The only two remaining failing checks are expected by design: `workflow-evidence-policy` (until this pre-merge line lands) and "Require ready-for-review PR" (until the PR body's `expected-head-sha` is updated to this commit, done alongside this same update).
