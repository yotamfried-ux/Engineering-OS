...
- steps: compared old branch, confirmed it was behind `main`, created `ops/readiness-coverage-audit` from current `main`, re-applied the audit and CI changes, opened PR 128, restored this plan after accidental edits, and left PR 127 as superseded.
- evidence: compare reported `behind_by: 0` for the clean replacement branch.
