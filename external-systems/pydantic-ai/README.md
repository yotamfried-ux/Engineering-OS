...
- **Tool functions returning non-serializable Python objects:** Every tool registered with `@agent.tool` or `@agent.tool_plain` must return a value that Pydantic AI can serialize to send back to the model (string, dict, list, or a Pydantic model). Returning a raw Python object like a `datetime`, a custom class instance, or a database cursor raises a `ValidationError` at runtime. Convert return values to a string or a Pydantic model before returning.
- **Using `result_type` without a system prompt instructing the shape:** Setting `result_type=MyModel` tells Pydantic AI to validate and parse the model's output, but the underlying LLM only knows what shape to produce if the system prompt explicitly describes the expected structure. Without clear instructions the model often returns free-form text that fails validation and triggers retries. Include explicit field descriptions in the system prompt or via Pydantic `Field(description=...)` annotations.
- **`RunContext` not passed as first argument to `@agent.tool` functions:** Tools registered with `@agent.tool` must accept a `RunContext[DepsType]` as their first positional argument; this is how Pydantic AI injects dependencies and message history. Omitting it or placing it in a non-first position causes a `TypeError` when the tool is invoked. Use `@agent.tool_plain` if you do not need the context.
- **Async tools called from a sync agent run:** Defining a tool as `async def` and then running the agent with `agent.run_sync()` in a context that already has a running event loop (e.g., inside Jupyter or another async framework) raises a `RuntimeError: This event loop is already running`. Either make all tools synchronous when using `run_sync`, or use `await agent.run()` from within an async context throughout.
- **Retrying on model validation errors without a limit:** By default, when the model returns output that fails `result_type` validation, Pydantic AI retries the call with the validation error appended to the conversation. Without setting `retries=N` on the agent, a persistently confused model can retry indefinitely, accumulating cost. Always set `retries=3` (or a suitable low cap) to surface the issue quickly rather than burning tokens on repeated failed attempts.
