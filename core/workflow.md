...
### Plan Timestamp — מתי ה-plan היה עדכני

שער הטריות (48h כברירת מחדל) חייב לדעת **מתי ה-plan נכתב**. הוא **אינו** משתמש ב-mtime
של מערכת הקבצים: `git clone` כותב לכל קובץ מעקב את זמן ה-checkout, ולכן plan בן חודשיים
מדווח כבן 0 שעות ו-`ls -t` בוחר plan שרירותי. זהו ה-counterpart הזמני של KG7
ב-[`hooks-policy.md`](./hooks-policy.md): KG7 מכסה plan טרי אך לא רלוונטי; כאן מדובר
ב-plan ישן שמדווח כטרי.

המקור הקנוני הוא [`../scripts/enforcement/lib/plan-time.sh`](../scripts/enforcement/lib/plan-time.sh),
וסדר ההכרעה שלו:

1. **plan שכבר ב-git ונקי בעץ העבודה** — זמן ה-commit האחרון שנגע בנתיב. היסטוריית git
   שורדת clone.
2. **plan שאינו ב-git, או שיש בו שינויים לא-מקומיטים** — השדה שה-plan מצהיר בעצמו:

   ```md
   | Plan Timestamp | 2026-09-02T00:41:02Z |
   ```

   (גם הצורה `Plan Timestamp: 2026-09-02T00:41:02Z` נתמכת.) חייב להיות ISO-8601 ב-UTC.
3. **אחרת** — חסימה. timestamp חסר, לא תקין, או **עתידי** (שהיה הופך את ה-plan לטרי לנצח)
   נדחה במפורש. אי-אפשר לדעת = חסימה, לא "טרי".

**כלל מעשי:** plan חדש חייב לכלול `Plan Timestamp`. אחרי שה-plan מקומיט, git מספק את
הזמן; אם עורכים plan ישן, רעננו את ה-`Plan Timestamp` שלו באותה עריכה.
