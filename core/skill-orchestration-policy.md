...
| **rtk** | ✅ **כן, בכל פרויקט** | חוסך 60–90% על פלטי Bash (git, grep, test). מותקן גלובלית פעם אחת (`rtk init -g`); SessionStart hook מוודא נוכחות אוטומטית. עלות אפסית לאחר ההתקנה. |
...
| **ui-ux-pro-max** | ⚠️ **מותנה — רק לפרויקט עם UI** | מחליף את frontend-design: 67 UI styles, 161 palettes, 57 font pairings, 15 tech stacks. Claude Code plugin. |
| ~~frontend-design~~ | ❌ **DEPRECATED** | הוחלף ב-ui-ux-pro-max. Level הורד ל-0 — אין לבחור אותו בפרויקטים חדשים. |
...
superpowers · security-review · graphify · rtk · claude-mem. הסקריפט מסמן את אלה כ-`default`,
את ui-ux-pro-max/claude-code-workflows כ-`conditional`, ואת gstack כ-`opt-in`. frontend-design מסומן L0 (deprecated).
...
**סקילים הם כלים שמיישמים שלבי `workflow.md` — לא מחליפים אותם.**
הכלל חל על **סקילי משימה** בלבד (ראה הבחנה מיד). השלב חייב לקרות; הסקיל הוא האמצעי.

### שני סוגי סקילים — סדר הפעלה שונה

**סקילי תשתית** (`context-optimization` · `memory`) — **לפני ה-workflow ולאורכו:**
רצים בתחילת כל סשן ומשרתים את כל השלבים. אין להם "שלב ייעודי" ב-workflow:
- **graphify** — בונה גרף קוד מיד כשסשן נפתח; שולף תת-גרף לפני כל קריאת קובץ.
  חוסך קונטקסט לאורך כל העבודה ולכן רץ **תמיד, ללא יוצא מן הכלל**.
- **claude-mem** — משחזר הקשר מסשנים קודמים; פסיבי ורץ מתחת לכל ה-pipeline.

**סקילי משימה** (`planning` · `coding` · `security` · `review`) — **שלב 3 ב-workflow.md:**
מוערכים לאחר אפיון (שלב 1) ואיסוף מידע (שלב 2). ניתן ואף רצוי להשתמש בהם
לביצוע שלבי ה-workflow עצמם (למשל: `superpowers:brainstorming` לכתיבת התוכנית ב-Notion).

### Execution Pipeline (סקילי משימה בלבד)
...
| [ui-ux-pro-max](../external-skills/ui-ux-pro-max/) | ui-ux, coding | **L2** ל-UI / L1 אחרת | ⚠️ מותנה (UI) | Claude Code plugin marketplace |
| ~~[frontend-design](../external-skills/frontend-design/)~~ | ui-ux, coding | **L0** (deprecated) | ❌ DEPRECATED | השתמש ב-ui-ux-pro-max |
