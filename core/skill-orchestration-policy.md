...
הערכת סקילים היא **שלב 3** ב-[`workflow.md`](./workflow.md) › `<workflow>` —
אחרי אפיון (שלב 1: Notion) ואחרי איסוף מידע (שלב 2: Context7 + patterns/), לא לפניהם.

ניתן ואף רצוי להשתמש בסקילים **לביצוע** שלבי ה-workflow עצמם:
- `superpowers:brainstorming` / `superpowers:writing-plans` לכתיבת תוכנית ב-Notion
- Context7 לאיסוף תיעוד רשמי (שלב 2)
- `superpowers:verification-before-completion` לפני קומיט/מיזוג

אבל **השלב חייב לקרות** גם אם הסקיל לא מותקן. הסקיל הוא האמצעי; השלב הוא החובה.

**לפני ביצוע משימה, קלוד חייב להעריך את הסקילים הזמינים** (שלב 3 בלבד — לאחר אפיון ואיסוף מידע).
