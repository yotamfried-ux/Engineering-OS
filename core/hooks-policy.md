...
- **`enforce-quality.sh`** (quality-gates.md) — `pre-commit`: חוסם הוספת שאריות debug ל-diff ה-staged (JS/TS/Python/Ruby: `debugger`/`breakpoint()`/`pdb`/`pry`/`byebug`; Go: `runtime.Breakpoint()`/`debug.PrintStack()`/`spew.Dump()`; Rust: `dbg!()`; Java/Kotlin: `dumpStack()`) + סמני merge-conflict. אזהרה לא-חוסמת: `console.log`/`print`/`fmt.Printf`/`log.Printf`/`eprintln!()`/`System.out.print`. bypass: `EOS_BYPASS_CLEANUP=1`, master `EOS_BYPASS_QUALITY=1`. (lint+test וסריקת test-files נשארים ב-pre-commit.sh.)
