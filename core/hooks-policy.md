...
- **`enforce-workflow.sh`** (workflow.md) — PreToolUse: שער כתיבה (plan), Context7 לפני install, tasks.json (כולל schema validation: מערך `tasks` עם `id`/`title`/`status`) לפני Agent. bypass: `EOS_BYPASS_WORKFLOW=1`, `EOS_BYPASS_TASKSJSON=1` (לשער tasks.json בלבד).
...
אמורים לפעול וחסימת `exit 1` אמורה להישמר — אך כשל שקט אפשרי אם:
- `python3` אינו מותקן ב-runner (גורם לparse-failure שב-enforce-workflow.sh גורר `exit 0`)
- פקודת ה-hook עטופה ב-`|| true` (מבטל את exit code)
- מטא-נתוני ה-JSON הנכנסים לא תואמים את ה-schema הצפוי

אם hook אמור לחסום אבל לא חסם — **אל תניח שהאכיפה תקינה**; בדוק ש`python3` זמין
ושאין `|| true` בסוף פקודת ה-hook.
