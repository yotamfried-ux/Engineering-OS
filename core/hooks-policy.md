...
- **דוח שימוש בסוף turn** — Stop hook שמזכיר/מאמת שצורף בלוק "🧰 במה השתמשתי"
  (קונקטורים, MCP, סקילים, patterns, תיעוד) **ו-"🔍 ביקורת עצמית"** (כיול: L2 שדולגו,
  נהלים שלא מומשו, פעולות שהופרו). ה-Stop hook מציג רשימת-כיול שClaude משווה אליה ומדווח
  בכנות. מגבה את [`CLAUDE.md`](../CLAUDE.md) › `<communication>`.
