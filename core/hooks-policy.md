...
- **`enforce-quality.sh`** (quality-gates.md) — `pre-commit`: חוסם הוספת שאריות debug ל-diff ה-staged (`debugger`/`breakpoint()`/`pdb`/`pry`/`byebug` + סמני merge-conflict); `console.log`/`print` → אזהרה לא-חוסמת. bypass: `EOS_BYPASS_CLEANUP=1`, master `EOS_BYPASS_QUALITY=1`. (lint+test וסריקת test-files נשארים ב-pre-commit.sh.)
