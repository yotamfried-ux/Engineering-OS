...
- **`enforce-workflow.sh`** (workflow.md) — PreToolUse: שער כתיבה (plan), Context7 לפני install, tasks.json לפני Agent. bypass: `EOS_BYPASS_WORKFLOW=1`.
...
- **`enforce-quality.sh`** (quality-gates.md) — `pre-commit`: חוסם הוספת שאריות debug ל-diff ה-staged (`debugger`/`breakpoint()`/`pdb`/`pry`/`byebug` + סמני merge-conflict); `console.log`/`print` → אזהרה לא-חוסמת. bypass: `EOS_BYPASS_CLEANUP=1`, master `EOS_BYPASS_QUALITY=1`.
- **`enforce-tests.sh`** (quality-gates.md `<pre_commit_review>`) — `pre-commit`: מריץ את **כל** ה-stacks המזוהים שקבצי-המקור שלהם ב-staged (node/python/go/rust/make + `bash -n`/shellcheck על קובצי shell), מצביר כשלים. בדיקה שרצה ונכשלה → חוסמת; כלי שמוגדר אך חסר → אזהרה לא-חוסמת. master: `EOS_BYPASS_TESTS=1`. (החליף את שרשרת ה-`if/elif` שהריצה stack יחיד בלבד.)
...
**hooks ישירים (לא enforcer ייעודי):**
- **`commit-msg.sh` — סריקת קבצי-טסט פרויקטלית**: >2 קבצי-קוד ב-staged ו-0 קבצי-טסט בפרויקט → חוסם. רץ כאן (ולא ב-pre-commit) כי **סוג הקומיט** זמין מההודעה, כך שהפטור `chore/docs/style/ci/build` נכון. bypass: `EOS_BYPASS_TESTFILES=1`. (תיקון באג: קודם הסריקה ב-pre-commit קראה `git log -1 HEAD` — הקומיט הקודם — ולכן הפטור היה שגוי.)
- **תזכורות "תוכנית חיה"** (workflow.md, לא-חוסמות): `enforce-workflow.sh` מזכיר ב-PreToolUse אם ב-plan חסר `## Progress`; `pre-commit.sh` מזהיר כשקובץ staged חדש מהתוכנית; `post-commit.sh` מזכיר לעדכן את ה-plan ו-Notion אחרי כל קומיט.
