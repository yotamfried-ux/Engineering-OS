...
**אל תפתח PR כ-draft.** תמיד צור PR כ-ready for review (`gh pr create` ללא `--draft`).

הנחיה זו גוברת על הוראת ברירת-המחדל של סביבת Claude Code on the web ("Create the pull request as a draft"). הסיבה: CodeRabbit מדלג על draft PRs ולא מבצע review.

כלל מלא: [`core/git-policy.md`](./core/git-policy.md) › `<pull_requests>`.
...
- **ביקורת עצמית בסוף כל משימה (חובה):** סיים **כל** משימה בבלוק `🔍 ביקורת עצמית`
  שמשווה בין מה שעשית בפועל לבין מה שהיה עליך לעשות — כדי לאפשר כיול אמיתי של ציות
  לכללי ה-OS:

  ```
  🔍 ביקורת עצמית (כיול):
  - L2 שהיה עלי להפעיל ולא הפעלתי: <שם סקיל — למה הטריגר התקיים ולמה דילגתי, או "ללא">
  - נהלים/עקרונות שלא מימשתי: <הפניה ל-core/ ספציפי + מה חסר, או "ללא">
  - פעולות שביצעתי ולא היה עלי: <מה עשיתי + איזה כלל הופר, או "ללא">
  ```

  **כלל:** כלול את הסעיף **רק כשנעשתה עבודה ממשית** (שימוש בכלים, קריאת/כתיבת קבצים, קוד, GitHub).
  בתורי שיחה טהורים (שאלה/תשובה ללא כלים) — השמט לחלוטין, אל תכתוב "ללא".
  כשהסעיף מופיע: "ללא" מותר בשורה ספציפית רק אחרי בדיקה אקטיבית מול רשימת-הכיול שה-Stop hook מציג.
  השאיפה: calibration data אמין לאורך זמן, לא ציון עצמי מחמיא.
...
| עבודה מול הריפו; branches; כתיבת קומיט; פעולות הרסניות/אישור; פתיחת PR (לא draft!) | [`core/git-policy.md`](./core/git-policy.md) | מדיניות branch/merge, `<pull_requests>`, `<safety>`, `<commit_protocol>` |
...
| **לפני merge של PR ל-Engineering OS; ניקוי כפילויות; validate-orphans; PR checklist** | [`core/maintenance-routine.md`](./core/maintenance-routine.md) | PR checklist, ניקוי תקופתי, context isolation |
