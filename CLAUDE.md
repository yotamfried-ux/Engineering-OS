...
- **דוח שימוש בסוף כל משימה (חובה):** בנוסף לדיווח הרגיל (מה בוצע / עובד / לא עובד /
  ...
  **כלל לסעיף `🔍 ביקורת עצמית`:** חובה כמו שאר הסעיפים — לא אופציונלי.
  "ללא" מותר רק אחרי בדיקה אקטיבית מול רשימת-הכיול שה-Stop hook מציג.
  Stop hook מציג את הרשימה (L2 triggers + נהלים) — השווה אותה למה שעשית בפועל.
