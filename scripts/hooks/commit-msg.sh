...
# Lives here (not pre-commit) because the commit TYPE is known from the message, so
# chore/docs/style/ci/build are exempted correctly. Bypass: EOS_BYPASS_TESTFILES=1.
case "${EOS_BYPASS_TESTFILES:-}" in 1|true|TRUE|yes|YES) exit 0 ;; esac
TYPE=$(printf '%s' "$MSG" | head -1 | grep -oE '^[a-z]+' || true)
case "$TYPE" in chore|docs|style|ci|build) exit 0 ;; esac
...
if [ "${STAGED_CODE:-0}" -gt 2 ]; then
  ...
  if [ "${PROJECT_TESTS:-0}" -eq 0 ]; then
    ...
    echo "   (Project-wide scan — any test file anywhere satisfies it; not per-file coverage.)"
