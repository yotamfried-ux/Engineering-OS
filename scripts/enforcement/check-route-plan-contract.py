...
WAIVER_RE = re.compile(r"\b(waiver|known[ -]?gap|gap:|explicit exemption|planned requirement)\b", re.I)
...
def field_value(text: str, names: list[str]) -> str:
    ...
    for line in text.splitlines():
        ...
        if "|" in raw:
            ...
            labels = [normalize_label(c) for c in cells]
            for i, label in enumerate(labels[:-1]):
                if label in wanted:
                    return strip_md_value(cells[i + 1])
...
def validate_plan(plan_path: Path, targets: list[str]) -> list[str]:
    ...
    if redaction and not re.search(r"redact|metadata|sensitive|exclude", redaction, re.I):
        failures.append(f"{plan_path}: evidence_redaction_rule must state how evidence is redacted or excluded.")
