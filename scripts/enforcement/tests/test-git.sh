...
expect "EOS_BYPASS_MAINPUSH env-only request does not skip G3" 1 "$(EOS_BYPASS_MAINPUSH=1 run 'git push origin main')"
...
expect "EOS_BYPASS_CONTENTS_API env-only request does not skip G4" 1 "$(EOS_BYPASS_CONTENTS_API=1 run 'gh api repos/o/r/contents/file.md --method PUT -f message=x -f content=y')"

echo "── bypasses + general ──"
expect "EOS_BYPASS_FORCEPUSH env-only request does not skip G1"  1 "$(EOS_BYPASS_FORCEPUSH=1 run 'git push --force')"
expect "EOS_BYPASS_DRAFTPR env-only request does not skip G2"    1 "$(EOS_BYPASS_DRAFTPR=1 run 'gh pr create --draft')"
expect "EOS_BYPASS_GIT master request is denied" 2 "$(EOS_BYPASS_GIT=1 run 'git push --force')"
