...
with tempfile.TemporaryDirectory() as td:
    ...
    class DelayedCompletionProvider(authorizer.FixtureProvider):
        def __init__(self, fixture):
            super().__init__(fixture)
            self.consumer_run_reads = 0

        def workflow_run(self, full_name: str, run_id: int):
            run = super().workflow_run(full_name, run_id)
            if run_id == 601:
                self.consumer_run_reads += 1
                if self.consumer_run_reads == 1:
