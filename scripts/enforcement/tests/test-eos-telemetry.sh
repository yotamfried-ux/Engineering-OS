...
PAYLOAD='{"tool_name":"Bash","tool_input":{"command":"npm install example-package --token SECRET_SHOULD_NOT_APPEAR","file_path":"src/private/customer-secret-file.ts"}}'
...
failcase raw_secret_not_recorded grep -q 'SECRET_SHOULD_NOT_APPEAR' "$TMP/.engineering-os/telemetry/events.jsonl"
failcase raw_command_not_recorded grep -q 'npm install example-package' "$TMP/.engineering-os/telemetry/events.jsonl"
failcase raw_path_not_recorded grep -q 'src/private/customer-secret-file.ts' "$TMP/.engineering-os/telemetry/events.jsonl"
