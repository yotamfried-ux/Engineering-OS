...
req(len(checkouts)>=3,'product, trusted base policy, and telemetry checkouts must be pinned to full SHAs')
req('ref: ${{ env.PR_HEAD_SHA }}' in text,'product checkout must use exact live PR head')
req('ref: ${{ env.PR_BASE_SHA }}' in text,'trusted telemetry policy checkout must use exact live PR base SHA')
req('path: .engineering-os/trusted-policy-source' in text,'trusted telemetry policy must be isolated from the PR checkout')
req('sparse-checkout: .engineering-os/telemetry-policy.json' in text,'trusted checkout must fetch the base telemetry policy')
req('trusted_policy=".engineering-os/trusted-policy-source/.engineering-os/telemetry-policy.json"' in text,'selector must name the isolated trusted base policy path')
req('if [ -e "$trusted_policy" ] || [ -L "$trusted_policy" ]; then' in text,'selector must distinguish an absent base policy from an invalid present entry')
req('[ ! -f "$trusted_policy" ] || [ -L "$trusted_policy" ]' in text,'present trusted base policy must be a regular non-symlink file')
req('"schema_version":"eos.telemetry.policy.v1"' in text and '"mode":"disabled"' in text,'workflow must generate a schema-valid trusted disabled policy when the base policy is absent')
req('--policy-file "$trusted_policy"' in text,'selector must always receive an explicit trusted base policy file')
req('policy_args' not in text,'conditional policy argument fallback must not return')
