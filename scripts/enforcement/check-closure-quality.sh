...
need_path(){ local h="$1" label="$2"; sec "$file" "$h" | grep -Eq '(scripts|tests|test|src)/[^ `)\]]+' || { echo "closure check failed: $label path missing" >&2; exit 1; }; }
...
need_path 'טסט רגרסיה' regression
echo ok
