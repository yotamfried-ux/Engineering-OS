...
- [ ] Fail CI when a new project type appears in templates, docs, route plans, or roadmap files without all required registry mappings. — only partially true: enforced for unregistered `templates/<id>/` directories and for project types referenced in `result-loop-requirements.tsv`/`documentation-sources.tsv`/`pattern-requirements.tsv`/`skill-requirements.tsv` without a roadmap row. Confirmed via review that the checker does **not** scan `docs/` prose or `.claude/plans/*.md` Route Plan fields (e.g. a Route Plan naming an unregistered `selected_project_type` with no registry footprint passes silently) — same gap already correctly noted as unchecked under "Enforcement implementation" below; left unchecked here too for consistency.
