...
Purpose: make long AI development sessions result-driven instead of instruction-driven. A project task is not considered controllable until the selected project type has a contract that tells the AI how to run, observe, test, measure, monitor, export telemetry, and repair the work.
...
5. `feedback_surfaces` — visual, operational, logical, console/network/log feedback.
...
10. `evidence_artifacts` — exact files/URLs/artifacts to preserve for review.
