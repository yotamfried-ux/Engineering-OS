...
15. **Governance evidence** — review and merge evidence closed by `check-pr-review-evidence.sh`'s extracted, fixture-tested validator (shallow/vague review evidence blocked; a deterministic `## Merge Readiness` PR-body artifact with base/expected-head-sha/ci/threads/approval is required and cross-checked against the live head SHA).
16. **Install downstream behavior** — closed by a manifest-driven policy-gate dependency copy (fixing the script-dependency bug PR D's review found across all affected workflows, not just the one it flagged) plus downstream-behavior fixtures for tool-absence fallback, command parsing, the learning-loop fix-needs-test gate (which was silently no-op in every installed target until this PR fixed `commit-msg.sh`'s reference-path resolution), templates/patterns reachability, and the tool-environment contract inside a generated target.
