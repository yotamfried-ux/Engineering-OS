...
| Claude run trace / experiment log | Partially enforced | Gate: workflow/connector evidence policies. Owner: trace-governance. Evidence: workflow-evidence-policy, connector-evidence-policy, run-trace gate, and direct run-trace waiver fixture coverage. | Not all significant agent runs are forced yet. |
| Positive/negative simulations | Partially enforced | Gate: `check-simulation-coverage.sh`. Owner: validation-governance. Evidence: `simulation-coverage.tsv`, extension rows under `simulation-coverage.d/`, `coverage-required-gates.tsv`, and `test-simulation-coverage.sh` now reject stale future/pending coverage language. | Required gates are manifest-backed; remaining work is replacing explicit coverage waivers with dedicated fixtures where feasible. |
...
10. **Simulation completeness** — maintained by `simulation-coverage.tsv`; stale future/pending coverage notes are now blocked, and remaining work is to replace explicit by-design coverage waivers with dedicated fixtures where feasible.
