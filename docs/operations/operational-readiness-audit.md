...
15. **Governance evidence** — review and merge evidence closed by `check-pr-review-evidence.sh`'s extracted, fixture-tested validator (shallow/vague review evidence blocked; a deterministic `## Merge Readiness` PR-body artifact with base/expected-head-sha/ci/threads/approval is required and cross-checked against the live head SHA); gap:install-downstream-behavior (install evidence) remains open for PR E.
