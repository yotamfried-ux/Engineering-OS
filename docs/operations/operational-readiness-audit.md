...
| Runtime performance metrics and monitoring | Missing enforcement | Gate: gap:performance-runtime-monitoring. Owner: observability-governance. Evidence: non-closed gap row in `docs/operations/known-gaps.tsv`; no validated runtime dashboard, telemetry baseline, or target-project performance evidence exists yet. | P0 gap: Engineering OS can pass policy/fixture gates without measuring actual behavior over real target-project work — duration, tool-call count, connector success rate, retry/fix loop depth, template/pattern reuse quality, eval pass rate, CI pass rate, and post-merge issue rate. |
...
1. **Runtime performance metrics and monitoring** — open P0. Engineering OS currently has policy gates, simulations, run-trace requirements, and CI evidence, but no validated monitoring surface or metrics baseline proving actual target-project behavior over time. Required next step: define and collect task duration, tool-call count, connector/tool success and failure rates, retry/fix loop depth, template/pattern reuse quality, eval pass rate, CI pass rate, post-merge issue rate, and regression rate across real target projects such as `project-8`.
