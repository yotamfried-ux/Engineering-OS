...
| Result Loop Contract enforcement | Enforced | Gate: `check-result-loop-contract.py` plus `result-loop-requirements.tsv`. Owner: ops-readiness. Evidence: merged PR #220 (`014c58f`); `scripts/enforcement/tests/test-result-loop-contract.sh` passes with positive and negative fixtures; wired into CI via the `scripts/enforcement/tests/test-*.sh` glob in `enforcement-tests.yml`. | Telemetry linkage and real-run evidence remain covered by separate gaps (`monitoring-metrics-sufficiency`, `project-8-real-run-evidence`). |
| Scaling extension enforcement | Enforced | Gate: `check-scaling-extension.py` plus `project-type-roadmaps.tsv` and `waiver-requirements.tsv`. Owner: ops-readiness. Evidence: merged PR #219 (`f19ce56`); `scripts/enforcement/tests/test-scaling-extension.sh` passes, rejecting incomplete-extension fixtures; wired into CI via the `scripts/enforcement/tests/test-*.sh` glob in `enforcement-tests.yml`. | Registry coverage backing new extensions remains tracked separately (`registry-coverage-backfill`). |
...
1. **Registry/manifest coverage backfill** — open gap; connectors alone show ~17 manifest rows against 49 `external-systems/` directories, so newer or unlisted assets can silently skip required-source selection until a follow-up PR closes the gap.
2. **Monitoring metrics sufficiency** — open gap; the telemetry exporter/importer/analyzer pipeline is implemented and tested, but no real target-project run has been imported yet, so sufficiency cannot be claimed.
3. **Project 8 real-run evidence** — blocked gap; the real-run experiment has not been performed and is explicitly out of scope for audit-reconciliation work; readiness claims must not get ahead of this.
4. **Selection coverage hardening** — closed by inventory-tied selection manifests, the registry-driven pattern gate, and the staged-change capability guard; maintain the manifests as inventories grow.
5. **Coverage map hardening** — covered by `coverage-required-gates.tsv`; maintain it whenever new gates are added.
6. **RTK runtime hardening** — covered structurally by RTK usage impact evidence and session setup checks; maintain it when RTK signals change.
7. **Route Plan quality gate** — closed structurally by concrete source and target relevance checks.
8. **Template/pattern rating lifecycle** — closed structurally by exact declared asset coverage and feedback evidence.
9. **Learning closure gate** — covered by `enforce-learning-capture.sh`; maintain content-quality fixtures when the lesson schema changes.
10. **Progress lifecycle** — covered by ordered progress lifecycle evidence.
11. **Graphify context graph** — covered by target-linked graph usage evidence.
12. **Connector correctness** — source/action/result/decision evidence, concrete result identifiers, and target linkage are enforced by `check-connector-evidence.sh`.
13. **Simulation completeness** — maintained by `simulation-coverage.tsv`.
14. **Post-merge validation** — covered by safe fake-gh repair issue simulation; live failures use the incident checklist.
15. **Documentation hygiene** — covered by `check-documentation-hygiene.sh`.
16. **Semantic cleanup** — covered by CI policy gates.
17. **Trace and test contracts** — covered by significant-run and missing-tool environment contracts.
18. **Governance evidence** — review and merge evidence are covered by `check-pr-review-evidence.sh`.
19. **Install downstream behavior** — covered by manifest-driven policy-gate dependency copy and generated-target fixtures.
