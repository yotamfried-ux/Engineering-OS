...
| Canonical telemetry trust boundaries | Enforced | Gate: telemetry-handoff-tests. Owner: ops-readiness. Evidence: merged PR #253 and exact-head regressions. | Direct archive import does not yet prove the same integrity contract. |
...
| Merge readiness exact-head and latest-attempt evidence | Missing enforcement | Gate: `check-merge-readiness.sh`. Owner: merge-governance. Evidence: workflow-run metadata. | gap:merge-readiness-exact-head-and-attempt-ordering — an older or wrong-head success can be mistaken for current evidence. |
...
- [ ] Reconcile `scripts/enforcement/MANIFEST.tsv` with the active capability registry and add a regression that rejects non-runtime wording for an enabled runtime gate.
- [ ] Reconcile telemetry plan, preflight, checklist, and analyzer terminology so first-run sufficiency and longitudinal sufficiency remain separate.
- [ ] Assign one owner for every active runtime, inventory, review, lifecycle, and readiness claim; derived documents must reference rather than duplicate volatile state.
- [ ] Extend bidirectional hygiene fixtures to reject every identified contradiction without rewriting historical plans.
