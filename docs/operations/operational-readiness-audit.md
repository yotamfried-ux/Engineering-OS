...
| Positive/negative simulations | Partially enforced | Gate: `check-simulation-coverage.sh`. Owner: validation-governance. Evidence: `simulation-coverage.tsv`, extension rows under `simulation-coverage.d`, `coverage-required-gates.tsv`, and `test-simulation-coverage.sh` now reject stale deferred coverage language. | Required gates are manifest-backed; remaining work is replacing explicit by-design coverage waivers with dedicated fixtures where feasible. |
...
10. **Simulation completeness** — maintained by `simulation-coverage.tsv`; stale deferred coverage notes are now blocked, and remaining work is to replace explicit by-design coverage waivers with dedicated fixtures where feasible.
