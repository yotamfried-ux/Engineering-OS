...
The export bundle contains only manifest.json, events.jsonl, and latest-summary.md. A missing or empty events.jsonl fails unless --empty-run is passed explicitly.
...
Do not build a backend pipeline before the local export/import/analyze path is proven on real target-project runs.
