...
## מה קרה
Codex review on Expiriens-saas-0.9 PR #2 (a real downstream install of Engineering OS) found that the generated `security-review workflow` CI workflow sent only `diff[:12000]` to the external AI reviewer. Any PR whose patch exceeded 12000 characters had everything past that point excluded from the security review while the check still reported success — the downstream validation PR itself (an install commit with 28 files, ~2400 lines) was exactly such a PR.
...
## השערות שנבדקו
- The 12KB limit is a review API constraint that forces truncation — rejected: the API accepts multiple sequential calls; chunking the diff across calls reviews everything within the same per-call bound.
- The workflow never runs in practice so the slice is harmless — rejected: it runs on every downstream PR when the configured reviewer credential is configured; the skip path only covers the missing-key case.

## ראיה
Codex P2 finding on Expiriens-saas-0.9 PR #2 (`.github/workflows/security-review workflow` line 36); the pre-fix heredoc in `scripts/skill-bootstrap.sh` contained `{diff[:12000]}`; the new regression test fails on that generator (`fail: diff reviewed via single truncating slice diff[:12000]`, exit 1) and passes 7/7 on the fixed one, including a behavioral check that reassembled chunks equal the full diff and the tail content is present.
